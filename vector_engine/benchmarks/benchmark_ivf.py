...
def generate_data(n_vectors: int, n_queries: int, dimension: int) -> Tuple[List[str], np.ndarray, np.ndarray]:
    ...
    # L2 normalize vectors to ensure consistent cosine distance metrics
...
def run_benchmark() -> Dict[str, Any]:
    ...
    index = IVFIndex(n_clusters=32, dimension=dimension)
    
    # Measure build CPU and memory
    process.cpu_percent(interval=None) # Reset CPU tracking
    ...
    for q in query_vectors:
        ...
        res = index.search(q, top_k=10, nprobe=1)
