...
<p align="center">
  <a href="https://github.com/KeyZama10/Empher/actions/workflows/ci.yml">
    <img src="https://github.com/KeyZama10/Empher/actions/workflows/ci.yml/badge.svg" alt="CI Status" />
  </a>
...
For deep-dive developer logs, resume bullet points, and recruiter-specific challenges, check out the [Engineering Project Showcase](vector_engine/docs/project_showcase.md).

---

## 🎥 1-Command Live Cluster Demo

Navigate into `vector_engine` and spin up a sharded, replicated cluster with etcd service discovery and Redis caches, insert vector embeddings, and demonstrate lookup cache hits in **under 15 seconds**:
...
![Recall vs Latency](vector_engine/docs/benchmarks/plots/recall_vs_latency.png)

### 8.2. HNSW vs IVF Core Performance Trade-off
![HNSW vs IVF Core](vector_engine/docs/benchmarks/plots/hnsw_vs_ivf.png)

### 8.3. Product Quantization Space Compression
![PQ Compression](vector_engine/docs/benchmarks/plots/pq_compression.png)

### 8.4. IVF nprobe parameter sweeps
![nprobe Tradeoffs](vector_engine/docs/benchmarks/plots/nprobe_tradeoff.png)

### 8.5. Distributed Cluster Scaling
![Distributed Scaling](vector_engine/docs/benchmarks/plots/distributed_scaling.png)

### 8.6. Replica Read Scaling (Primary vs Balanced)
![Replica Scaling](vector_engine/docs/benchmarks/plots/replica_scaling.png)

### 8.7. Crash Recovery State Reconstruction Speed
![Recovery Time](vector_engine/docs/benchmarks/plots/recovery_time.png)
...
1. **Execute Benchmarking Framework**:
   - Runs sequential sweeps for IVF, HNSW, PQ, nprobes, and horizontal scaling:
     ...
     ```
   - Generated reports are written under `vector_engine/docs/benchmarks/` as `benchmark_report.md`, `benchmark_results.csv`, and `benchmark_results.json`.
...
2. **Generate Publication-Quality Plots**:
   - Generates the 7 core performance charts:
     ...
     ```
   - Plots are written under [vector_engine/docs/benchmarks/plots/](vector_engine/docs/benchmarks/plots/). Check the [benchmark_summary.md](vector_engine/docs/benchmarks/benchmark_summary.md) for tables and linked visualizations.
...
1. Navigate to code package:
   ```bash
   cd vector_engine
   ```
2. Boot a primary worker on port 50091 and ingest 1,000 vectors.
3. Kill the process abruptly:
   ```bash
   kill -9 <worker_pid>
   ```
4. Reboot the worker node:
   ...
   ```
5. Query node status: The worker reads snapshot NPZs, replays trailing logs, and recovers 1,000 vectors with zero-data-loss.
